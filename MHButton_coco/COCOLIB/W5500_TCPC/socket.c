...
uint8 socket(SOCKET s, uint8 protocol, uint16 port, uint8 flag)
{
      ...
      )
   {
      ...
      if (port != 0) {
         ...
         g_w5500_local_port++; // if don't set the source port, set local_port number.
         IINCHIP_WRITE(Sn_PORT0(s) ,(uint8)((g_w5500_local_port & 0xff00) >> 8));
         IINCHIP_WRITE(Sn_PORT1(s) ,(uint8)(g_w5500_local_port & 0x00ff));
      }
...
{
    static uint8 cocoguojiaTimerOutChangeN=0;
    uint8 ret;
    if
        (
...
//            }
            //------------------------------------------------------------------------------------------------
            //超时管理 超过5s没有退出这个大循环 则break 原来超时管理貌似不起作用 cocoguojia say
            cocoguojiaTimerOutChangeN++;
            if(6<=cocoguojiaTimerOutChangeN)
            {
                cocoguojiaTimerOutChangeN=0;
                break;
            }
            else
            {
                osDelay(1000);
                App_Printf("$$connect2...->%d\r\n",cocoguojiaTimerOutChangeN);
            }
