...
//    HAL_StatusTypeDef returnDat;
    ...
    //----------------------------------------------------------------------------
    //因为这是初始化过程中的发送 所以不会与 wifi发送任务中的发送冲突
    HAL_UART_Transmit(&huart4, g_uart_tx_buf, len, 50);
       
//  returnDat=HAL_UART_Transmit(&huart4, g_uart_tx_buf, len, 50);   
//	do
//	{
//		returnDat=HAL_UART_Transmit(&huart4, g_uart_tx_buf, len, 10);
//        if(HAL_OK!=returnDat)
//        {
//            osDelay(10);
//        }
 ...
 * @retval      无
 */
void ATK_MW8266D_UART_IRQHandler(UART_HandleTypeDef *huart)
{
    uint8_t tmp;
    ...
    if(huart->Instance==USART4)
    {
        ...
        if (__HAL_UART_GET_FLAG(&huart4, UART_FLAG_IDLE) != RESET)       /* UART总线空闲中断 */
        {
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    if(huart->Instance==USART4)
    {
        if (__HAL_UART_GET_FLAG(&huart4, UART_FLAG_ORE) != RESET)        /* UART接收过载错误中断 */
        {
            ...
            (void)huart4.Instance->ISR;                                   /* 先读SR寄存器，再读DR寄存器 */
            (void)huart4.Instance->RDR;
        }
    }
}
